...
def makeInfo(): ### Makes Info object which stores program details, mainly for initial print to screen.
    ...
    (program, version, last_edit, copy_right) = ('RJE_PAF', '0.7.3', 'September 2019', '2019')
    description = 'Minimap2 PAF parser and converter'
    ...
    def invertCS(self,lentry):  ### Inverts a CS string local entry so that the query is now the hit anf vice versa
        ...
        try:### ~ [1] Setup ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            for cs in aln:
                # Replacements
                if cs[0] in replace:
                    if revcomp: cs = '%s%s' % (cs,rje_sequence.reverseComplement(cs[1:]))
...
#########################################################################################################################
    def pafToVariants(self):    ### Performs variant calling on parsed PAF (CS) data
        ...
        try:### ~ [1] Setup ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            #i# pafhead = Qry QryLen QryStart QryEnd Strand Hit SbjLen SbjStart SbjEnd Identity Length Quality .. cs
            pafdb = self.db('paf')
            rdb = db.addEmptyTable('rid',['Qry','QryStart','QryEnd','HitStart','Hit','Strand','cs'],['Qry','QryStart','QryEnd','HitStart','Hit'])
            ...
            for entry in pafdb.entries():
                rdb.addEntry(self.invertCS(entry))

            #i# Reads are given a RID based on revised QryStart
            rdb.newKey(['Qry','QryStart','QryEnd','HitStart','Hit'])    # This is just to get the right sorting
            ...
            #RID	Locus	Start	End     Name
            rdb.keepFields(['RID','Locus','Start','End','Name','Strand','cs'])
            rdb.saveTable()

            ### ~ [3] ~ Generate RID SNP table ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            minqn = 10
            indels = True #Whether to include indels in "SNP" parsing [True]
            skiploci = []  # List of loci to exclude from pileup parsing (e.g. mitochondria) []
            snptableout = True # Output filtered alleles to SNP Table [False]
            mincut = 0.05        # Minimum read count for minor allele (proportion if <1) [1]
            absmincut = 2     # Absolute minimum read count for minor allele (used if mincut<1) [2]
            biallelic = True   # Whether to restrict SNPs to pure biallelic SNPs (two alleles meeting mincut) [False]
            ignoren = True     # Whether to exclude "N" calls from alleles [True]
            ignoreref = False   # If False will always keep Reference allele and call fixed change as SNP [False]
            snpdb = db.addEmptyTable('snp',['Locus','Pos','Ref','N','QN','Seq','Dep','RID'])
            ...
            for locus in rdb.indexKeys('Locus'):
                ...
                for rentry in rdb.indexEntries('Locus',locus):
                    rid = rentry['RID']
                    for (qrypos,sbjpos,cs) in self.mapCS(rentry): varlist.append((qrypos,cs,rid))
                ...
                while varlist:
                    ...
                    posvar = {}     # cs:[RIDs]
                    while varlist[0][0] == pos:
                        (pos,rid,cs) = varlist.pop(0)
                        ...
                        if cs[0] == '*':  # Substitution: ref to query
                            pass
                        if cs[0] == '+':  # Insertion to the reference
                            pass
                    ...
                    if delrids: posvar['-'] = delrids[0:]
                    if matchrids: posvar[':'] = matchrids[0:]
                    varn += len(delrids)
                    varn += len(matchrids)
                    # Drop low abundance variants


                    # Apply indel filter
                    for cs in rje.sortKeys(posvar):
                        # Get rid of indels if indels=FALSE
                        if cs[0] in '+-' and not indels:
                            varn -= len(posvar.pop(cs))
...
# ~	[acgtn]{2}[0-9]+[acgtn]{2}	Intron length and splice signal
            ...
            ### ~ [4] ~ Save SNP Table ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            #i# Include headers containing key settings to check when re-loading data
            ## RID = Convert PAF table to inverse table
